...
The original plan for this milestone was a from-scratch prison-tutorial prologue world (opening scene + 3 in-prison minigames) plus a general polish pass. With roughly 6 days left of the original 9–10-day window, a brand-new hand-authored world is too much to responsibly finish, so the milestone was first re-scoped to drop the prologue world in favor of a smaller narrative capstone (a win cutscene) and a new 30-day clock mechanic. On a second pass, that capstone and clock were themselves cut: rather than spend the freed-up time on a scripted ending and a new pacing mechanic, it goes toward going deeper on three systems the game's own pitch already leans on hardest — crime, dating sim, and building interiors — on top of the prioritized six-item punch-list below. Naming and re-scoping this in the open, twice now, rather than quietly letting scope slip, is the same discipline this proposal already argues for elsewhere.

**Six-item punch-list, in priority order:**

1. **Storyline** — tighten the throughline that gives the player a reason to chase $10M, replacing the prison world's job of carrying the narrative.
...
4. **Crime system expansion (car theft to police to jail to bribe/puzzle)** — straighten the existing flow's sequencing/logic so the flee-or-bribe, then jail, then bribe-or-puzzle path plays correctly, then go further: broaden heist variety and sharpen cop AI/consequences rather than stopping at a fix pass.
...
6. **Dating sim expansion** — verify the existing Lisa dating/chat flow works end-to-end, then extend the romance/chat system to more of the 76-NPC roster rather than stopping at the one confirmed path.

**New: building-interiors art pass.** On top of the six-item list, use part of the freed-up time to run an interior art pass across the hub buildings (Tokyo Stock Exchange, Neon Dragon Casino, Bank & Realty Office, and the rest) so each hub's interior looks considered rather than placeholder, on top of the existing exterior/landmark art already shipped.

Item 5 is a quick verification pass and unlikely to slip. If time runs short, item 3 (additional casino minigames) is the first to compress — it's additive scope, not a fix to something already broken — followed by the building-interiors art pass, which is additive polish rather than a system fix. Item 1 (storyline) and the crime-system and dating-sim expansions (items 4 and 6) are not cut candidates: together with the existing systems, they're what the remaining time is now built around.

### Explicitly Out of Scope for This Milestone

The prison-tutorial prologue world (opening scene + 3 in-prison minigames) originally planned for this milestone is explicitly descoped — deferred as a possible future milestone if there's appetite for a full prologue world later, not abandoned as a concept. The win cutscene and the 30-day clock, both planned in an earlier revision of this milestone, are also now descoped in favor of going deeper on the crime, dating-sim, and building-interiors work above — deferred, not abandoned. Ambient background music, the decision on how far to extend the free-text NPC chat backend, the Food Center energy-top-up hook, a full Dock/Pier hub building (fishing itself already works as a standalone activity; a dedicated hub around it does not), and reviving the built-but-dormant Hunter's Rift / Yu-Gi-Oh / Domino City worlds (unreachable by design, not by bug) remain real, tracked backlog items but are not attempted here. The Entertainment Complex remains deferred by existing design decision. The Social-feed-to-price sentiment-engine tie-in, NPC vehicle traffic restoration, and ambient-roster expansion are deferred to a future milestone, not abandoned. Naming and deferring these is itself part of the scoping discipline this proposal is demonstrating.

### Success Criteria

- `npm run build` and `npm run lint` (oxlint) stay clean (0 errors).
- Crime system: the existing car-theft-to-jail flow plays correctly end-to-end, plus at least one new heist variant or cop-AI/consequence improvement beyond the existing set — confirmed in a live playthrough.
- Dating sim: the Lisa dating/chat flow works end-to-end, and the romance/chat system is confirmed working with at least one additional NPC beyond Lisa — confirmed live.
- Building interiors: a visible art pass lands on at least a handful of hub interiors (e.g., Tokyo Stock Exchange, Neon Dragon Casino, Bank & Realty Office) beyond placeholder — confirmed via screenshot.
- Each of the six punch-list items has a one-line record of what changed, verified live (screenshot or recorded), not claimed from a diff — including explicit confirmation that the church hub is complete and the dating-sim path works end-to-end.
- A recorded demo exists showing the crime-system expansion, the dating-sim expansion, and at least one polished interior, as raw material for the video-reel agent, and it matches what this proposal describes.
...
- **Re-scoping mid-milestone, twice now, can look like scope failure instead of scope discipline.** Mitigated by naming both cuts explicitly (see Out of Scope) — the prison world, then the win cutscene/30-day clock — rather than letting either quietly disappear.
- **Expanding three systems (crime, dating sim, interiors) in the same ~6 days as the six-item punch list risks doing everything shallowly rather than a smaller set well.** Mitigated by the pre-declared compression order above (casino minigames, then the interiors art pass, compress first; storyline and the crime/dating-sim expansions are not cut candidates).
- **The crime-system and dating-sim items are diagnostic before they're additive** — each starts with verifying an existing flow, which could turn out bigger once actually checked live, eating into the time meant for the "go further" half of the work. Mitigated by time-boxing the verification pass before committing to how much expansion is realistic.
